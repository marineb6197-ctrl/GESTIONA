...
function productCompleteness(p){const checks=[p.name,p.sku||p.barcode,p.category,p.supplier_id,p.unit,p.brand,p.description,num(p.minimum_stock)>0,num(p.ideal_stock)>0,num(p.package_price_excl_vat)>0,num(p.units_per_package)>0,p.location,p.consumption_unit];return Math.round(checks.filter(Boolean).length/checks.length*100)}
...
function updateProductCalculations(){const packagePrice=num($('pPackage').value),units=Math.max(num($('pUnits').value),1),saleIncl=num($('pSale').value),saleVat=num($('pSaleVat').value),target=num($('pTargetMargin').value),stock=num($('pStock').value),yieldPct=Math.max(num($('pYield')?.value),0.01);const cost=packagePrice/units,usableCost=cost/(yieldPct/100),saleEx=saleIncl/(1+saleVat/100),margin=saleEx-usableCost,pct=saleEx>0?margin/saleEx*100:0,recommendedEx=target>0&&target<100?usableCost/(1-target/100):0,recommendedIncl=recommendedEx*(1+saleVat/100);$('calcUnitCost').textContent=money(cost);if($('calcUsableCost'))$('calcUsableCost').textContent=money(usableCost);$('calcSaleEx').textContent=money(saleEx);$('calcMarginEuro').textContent=money(margin);$('calcMarginPct').textContent=pct.toFixed(1)+' %';$('calcRecommended').textContent=money(recommendedIncl);$('calcStockValue').textContent=money(stock*cost)}
...
function supplierReferenceOptions(selected=''){return '<option value="">Choisir...</option>'+suppliers.map(s=>`<option value="${s.id}" ${String(s.id)===String(selected)?'selected':''}>${esc(s.name)}</option>`).join('')}
function renderSupplierReferenceDraft(){const host=$('supplierReferencesBody');if(!host)return;host.innerHTML=supplierReferenceDraft.length?supplierReferenceDraft.map((r,i)=>`<tr><td><select data-sr-index="${i}" data-sr-field="supplier_id">${supplierReferenceOptions(r.supplier_id)}</select></td><td><input data-sr-index="${i}" data-sr-field="supplier_sku" value="${esc(r.supplier_sku||'')}" placeholder="Référence"></td><td><input data-sr-index="${i}" data-sr-field="package_label" value="${esc(r.package_label||'')}" placeholder="Ex. carton 24 x 33 cl"></td><td><input type="number" min="0" step="0.0001" data-sr-index="${i}" data-sr-field="package_price_excl_vat" value="${num(r.package_price_excl_vat)}"></td><td><button class="btn danger mini" type="button" data-remove-sr="${i}">✕</button></td></tr>`).join(''):'<tr><td colspan="5" class="empty">Aucune référence alternative.</td></tr>';host.querySelectorAll('[data-sr-index]').forEach(el=>{el.oninput=()=>{const i=Number(el.dataset.srIndex),f=el.dataset.srField;supplierReferenceDraft[i][f]=f==='package_price_excl_vat'?num(el.value):el.value}});host.querySelectorAll('[data-remove-sr]').forEach(btn=>btn.onclick=()=>{supplierReferenceDraft.splice(Number(btn.dataset.removeSr),1);renderSupplierReferenceDraft()})}
async function loadSupplierReferences(productId){supplierReferenceDraft=[];if(productId){const {data,error}=await sb.from('product_supplier_references').select('*').eq('product_id',productId).order('created_at');if(!error)supplierReferenceDraft=(data||[]).map(x=>({...x}))}renderSupplierReferenceDraft()}
async function saveSupplierReferences(productId){const {error:delError}=await sb.from('product_supplier_references').delete().eq('product_id',productId);if(delError&&delError.code!=='42P01')throw delError;const rows=supplierReferenceDraft.filter(r=>r.supplier_id||r.supplier_sku).map(r=>({organization_id:orgId,product_id:productId,supplier_id:r.supplier_id||null,supplier_sku:(r.supplier_sku||'').trim()||null,package_label:(r.package_label||'').trim()||null,package_price_excl_vat:num(r.package_price_excl_vat)}));if(rows.length){const {error}=await sb.from('product_supplier_references').insert(rows);if(error)throw error}}
function resetProduct(){ $('productForm').reset();$('productId').value='';$('pUnit').value='pièce';$('pPurchaseUnit').value='colis';$('pUnits').value='1';$('pPurchaseVat').value='21';$('pSaleVat').value='21';$('pFavorite').value='false';$('pStatus').value='active';$('pTrackLots').value='false';$('pExpiryType').value='none';$('pYield').value='100';$('pWaste').value='0';$('pConversionFactor').value='1';supplierReferenceDraft=[];$('deleteProductBtn').classList.add('hidden');$('duplicateProductBtn').classList.add('hidden');renderImagePreview();$('productModalTitle').textContent='Ajouter un produit';renderSupplierOptions();renderSupplierReferenceDraft();loadPriceHistory(null);updateProductCalculations()}
window.editProduct=async id=>{const p=products.find(x=>x.id===id);if(!p)return;resetProduct();$('productId').value=p.id;$('pName').value=p.name;$('pBrand').value=p.brand||'';$('pDescription').value=p.description||'';$('pImage').value=p.image_url||'';$('pSku').value=p.sku||'';$('pBarcode').value=p.barcode||'';$('pCategory').value=p.category||'';$('pSubcategory').value=p.subcategory||'';$('pSupplier').value=p.supplier_id||'';$('pPurchaseUnit').value=p.purchase_unit||'colis';$('pLeadTime').value=num(p.lead_time_days);$('pUnit').value=p.unit||'pièce';$('pStock').value=p.stock;$('pMin').value=p.minimum_stock;$('pIdeal').value=num(p.ideal_stock);$('pMax').value=num(p.maximum_stock);$('pPackage').value=p.package_price_excl_vat;$('pUnits').value=p.units_per_package;$('pPurchaseVat').value=p.purchase_vat;$('pSale').value=p.sale_price_incl_vat;$('pSaleVat').value=p.sale_vat;$('pTargetMargin').value=p.target_margin_percent??'';$('pLocation').value=p.location||'';$('pFavorite').value=String(!!p.favorite);$('pStatus').value=p.product_status||'active';$('pTrackLots').value=String(!!p.track_lots);$('pShelfLife').value=num(p.shelf_life_days);$('pExpiryType').value=p.expiry_type||'none';$('pConsumptionUnit').value=p.consumption_unit||'';$('pConversionFactor').value=num(p.conversion_factor)||1;$('pYield').value=p.yield_percent==null?100:num(p.yield_percent);$('pWaste').value=num(p.preparation_waste_percent);$('pAllergens').value=p.allergens||'';$('pNotes').value=p.notes||'';await loadSupplierReferences(id);$('deleteProductBtn').classList.remove('hidden');$('duplicateProductBtn').classList.remove('hidden');renderImagePreview();$('productModalTitle').textContent='Fiche produit 360°';updateProductCalculations();await loadPriceHistory(id);openModal('productModal')}
...
$('productForm').onsubmit=async e=>{e.preventDefault();const id=$('productId').value,old=id?products.find(x=>x.id===id):null,payload={organization_id:orgId,venue_id:selectedVenue==='all'?null:selectedVenue,supplier_id:$('pSupplier').value||null,name:$('pName').value.trim(),brand:$('pBrand').value.trim()||null,description:$('pDescription').value.trim()||null,image_url:$('pImage').value.trim()||null,sku:$('pSku').value.trim()||null,barcode:$('pBarcode').value.trim()||null,category:normalizeCategoryName($('pCategory').value)||null,subcategory:$('pSubcategory').value.trim()||null,product_status:$('pStatus').value,purchase_unit:$('pPurchaseUnit').value.trim()||'colis',lead_time_days:Math.max(0,Math.round(num($('pLeadTime').value))),unit:$('pUnit').value.trim()||'pièce',stock:num($('pStock').value),minimum_stock:num($('pMin').value),ideal_stock:num($('pIdeal').value),maximum_stock:num($('pMax').value),package_price_excl_vat:num($('pPackage').value),units_per_package:Math.max(num($('pUnits').value),1),purchase_vat:num($('pPurchaseVat').value),sale_price_incl_vat:num($('pSale').value),sale_vat:num($('pSaleVat').value),target_margin_percent:$('pTargetMargin').value===''?null:num($('pTargetMargin').value),location:$('pLocation').value.trim()||null,favorite:$('pFavorite').value==='true',track_lots:$('pTrackLots').value==='true',shelf_life_days:Math.max(0,Math.round(num($('pShelfLife').value))),expiry_type:$('pExpiryType').value,consumption_unit:$('pConsumptionUnit').value.trim()||null,conversion_factor:Math.max(num($('pConversionFactor').value),0.000001),yield_percent:Math.min(100,Math.max(0,num($('pYield').value))),preparation_waste_percent:Math.min(100,Math.max(0,num($('pWaste').value))),allergens:$('pAllergens').value.trim()||null,notes:$('pNotes').value.trim()||null};if(payload.maximum_stock>0&&payload.ideal_stock>payload.maximum_stock){toast('Le stock idéal ne peut pas dépasser le stock maximum');return}if(payload.ideal_stock>0&&payload.minimum_stock>payload.ideal_stock){toast('Le stock minimum ne peut pas dépasser le stock idéal');return}let res;if(id)res=await sb.from('products').update(payload).eq('id',id).select().single();else res=await sb.from('products').insert(payload).select().single();if(res.error){toast(res.error.message);return}try{await saveSupplierReferences(res.data.id)}catch(err){toast('Produit enregistré, mais les références fournisseurs nécessitent la migration V22');console.error(err)}const priceChanged=!old||num(old.package_price_excl_vat)!==payload.package_price_excl_vat||num(old.units_per_package)!==payload.units_per_package;if(priceChanged&&payload.package_price_excl_vat>0){await sb.from('product_prices').insert({organization_id:orgId,product_id:res.data.id,supplier_id:payload.supplier_id,package_price_excl_vat:payload.package_price_excl_vat,units_per_package:payload.units_per_package,purchase_vat:payload.purchase_vat,source:'manual',created_by:user.id})}await audit(id?'Produit 360° modifié':'Produit 360° créé','product',res.data.id,{name:res.data.name,price_changed:priceChanged,completeness:productCompleteness(res.data),supplier_references:supplierReferenceDraft.length});closeModal('productModal');await refresh();toast('Fiche produit enregistrée')};
$('addSupplierReferenceBtn')?.addEventListener('click',()=>{supplierReferenceDraft.push({supplier_id:$('pSupplier')?.value||'',supplier_sku:'',package_label:'',package_price_excl_vat:0});renderSupplierReferenceDraft()});
